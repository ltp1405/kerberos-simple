...
use messages::basic_types::{
    EncryptedData, EncryptionKey, KerberosTime, Microseconds, NameTypes, OctetString, PrincipalName,
};
use messages::{
...
pub fn receive_ap_reply(
    ...
    let session_key = client_env.get_tgs_reply_enc_part()?.key();
    ...
    if (&authenticator.ctime() != ap_rep_part.ctime() || &authenticator.cusec() != ap_rep_part.cusec()) {
